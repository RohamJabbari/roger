...
import { rogerTime, filteredData } from "$lib/stores/app";
      ...
      {#if speechBubbleData && verticalPositions}
        {#each speechBubbleData as d}
          {#if d.start_sec >= timelineScale.domain()[0] && d.end_sec <= timelineScale.domain()[1]}
            ...
            <div
              class="absolute group text-xs leading-3 rounded shadow-md hover:shadow-lg hover:z-50 transition-all ease-in-out"
              style="
